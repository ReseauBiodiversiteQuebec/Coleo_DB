-------------------------------------------------------------------------------
-- VIEW api.gabarit_zooplancton_long
-- DESCRIPTION Representation of the data for the zooplancton campaigns
-------------------------------------------------------------------------------

-- CREATE VIEW FOR zooplancton CAMPAIGN
-- View: api.gabarit_zooplancton_long

-- DROP VIEW api.gabarit_zooplancton_long;

CREATE OR REPLACE VIEW api.gabarit_zooplancton_long AS (
    ...
    left join lateral (
        select
            -- extra_variable_1
            CASE WHEN (extra #>> '{}')::jsonb -> 'date_de_denombrement' IS NOT NULL THEN 'date_de_denombrement' ELSE NULL END as observations_extra_variable_1,
            (extra #>> '{}')::jsonb -> 'date_de_denombrement' ->> 'value' as observations_extra_value_1,
            -- extra_variable_2
    ...
    where c.type = 'zooplancton'
    order by (c.opened_at, s.site_code, c.technicians, c.notes)
);

GRANT SELECT ON TABLE api.gabarit_zooplancton_long TO coleo_test_user;
GRANT ALL ON TABLE api.gabarit_zooplancton_long TO glaroc;
GRANT ALL ON TABLE api.gabarit_zooplancton_long TO postgres;
GRANT SELECT ON TABLE api.gabarit_zooplancton_long TO read_only_all;
GRANT SELECT ON TABLE api.gabarit_zooplancton_long TO read_only_public;
GRANT SELECT ON TABLE api.gabarit_zooplancton_long TO read_write_all;


-------------------------------------------------------------------------------
-- VIEW api.gabarit_zooplancton_short
-- DESCRIPTION Short representation of the data for the zooplancton campaign
-------------------------------------------------------------------------------

-- CREATE VIEW FOR PAPILIONIDÉS CAMPAIGN
-- View: api.gabarit_zooplancton_short

-- DROP VIEW api.gabarit_zooplancton_short;

CREATE OR REPLACE VIEW api.gabarit_zooplancton_short AS (
    select COALESCE(cl.name, '') || CASE WHEN s.site_name IS NOT NULL THEN ' - ' || s.site_name ELSE '' END as "Nom du site",
        ...
        o.depth as "Profondeur de l'échantillonnage",
...
GRANT SELECT ON TABLE api.gabarit_zooplancton_short TO coleo_test_user;
GRANT ALL ON TABLE api.gabarit_zooplancton_short TO glaroc;
GRANT ALL ON TABLE api.gabarit_zooplancton_short TO postgres;
GRANT SELECT ON TABLE api.gabarit_zooplancton_short TO read_only_all;
GRANT SELECT ON TABLE api.gabarit_zooplancton_short TO read_only_public;
GRANT SELECT ON TABLE api.gabarit_zooplancton_short TO read_write_all;
