...
CREATE MATERIALIZED VIEW api.taxa AS
    ...
    SELECT obs_lookup.id_taxa_obs,
     ...
     SELECT DISTINCT ON (group_lookup.id_taxa_obs) group_lookup.id_taxa_obs,
      ...
      WHERE v_lookup.match_type IN ('exact', 'partialexact') AND v_lookup.match_type <> 'complex'::text
  ...
  SELECT best_ref.id_taxa_obs,
    ...
    best_ref.rank,
    COALESCE(best_vernacular.vernacular_en, best_vernacular.vernacular_en, NULL) as vernacular_en,
    COALESCE(best_vernacular.vernacular_fr, best_vernacular.vernacular_fr, NULL) as vernacular_fr,
    obs_group.group_en,
    obs_group.group_fr,
